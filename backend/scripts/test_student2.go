...
func main() {
	...
	rows, err := pool.Query(context.Background(), "SELECT column_name FROM information_schema.columns WHERE table_name = 'students'")
	if err != nil {
		fmt.Printf("ERROR: %v\n", err)
	} else {
		defer rows.Close()
        fmt.Print("Columns in students table: ")
		for rows.Next() {
			var col string
			rows.Scan(&col)
			fmt.Printf("%s, ", col)
